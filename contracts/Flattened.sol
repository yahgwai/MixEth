...
*/
  function verify(uint256[12] params) public pure returns(bool) {
    ...
    require(ecmulVerify(params[0], params[1], u1, params[7], params[8]), "miss 1");
    require(ecmulVerify(params[2], params[3], u2, params[9], params[10]), "miss 2");
...
contract MixEthDemo is ERC223ReceivingContract {
    ParentLib lib;
  ...
  uint256 public amt = 0.1 ether; //1 ether in wei, the amount of ether to be mixed;
  uint256 public shufflingDeposit = 0.1 ether; // 1 ether, TBD
  ...
  */
  function uploadShuffle(address token, uint256[] _oldShuffle, uint256[] _shuffle, uint256[2] _newShufflingConstant) public onlyInShufflingPeriod(token) payable {
    //require(msg.value == shufflingDeposit+(_shuffle.length/2-Shuffles[token][shuffleRound[token]].noOfPoints)*shufflingDeposit, "Invalid shuffler deposit amount!"); //shuffler can also deposit new pubkeys
